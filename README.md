...
* **Fusión de Flujos Bidireccional:** Utiliza fases alternas *upstream* y *downstream* para colapsar la ventana de trabajo de manera agresiva sobre el núcleo desordenado del arreglo.
...
1.  **Escaneo de Adaptabilidad Local:** Al inicio de cada subproblema, se verifica si el segmento está ordenado o invertido. Si está invertido, se ejecuta una inversión directa de bloque ($O(n)$) y, en caso de que luego esté ordenado, se aborta la recursión innecesaria.
...
3.  **Combinación Bidireccional Avanzada (The Core Loop):** Mediante un bucle dinámico, se alternan dos operaciones especializadas:
    * **RCF (Reverse Contrary Flux):** Localiza rachas estrictamente descendentes y las invierte masivamente.
    * **OPF (Overlap Parallel Flows):** Funciona como un motor de arrastre lineal que alinea flujos paralelos y reduce de forma contundente las inversiones locales remanentes.

---

## 📊 Análisis de Complejidad y Rendimiento

Tras exhaustivas pruebas empíricas en vectores aleatorios y estructurados, las métricas de crecimiento han demostrado un comportamiento sub-cuadrático de alta eficiencia:
...
* `fluxSort(int[] arrayed, int top, int btm)`: Método recursivo principal que controla los límites de la ventana de operación y la subdivisión de tareas.
* `reverseContraryFlow(int limit, int dir, int aux)`: Escáner lineal encargado de detectar y neutralizar los flujos que contradicen el orden objetivo.
* `overlapParallelFlows(int limit, int dir, int aux)`: Optimizador de arrastre que desplaza elementos desalineados reduciendo las inversiones a un ritmo acelerado.
...
1.  Estrategias para mitigar o estabilizar el orden relativo de elementos idénticos en las fases OPF.
